...
| 2 | `chat_step_view` | 해당 단계 봇 메시지가 화면에 보일 때 | `step`: 단계 ID (welcome \| purpose \| topic \| gender \| age \| job \| **job_preference** \| interpreter \| location \| time \| when_join \| email), `step_index`: 0~11 | **이탈 구간**: step_index별 도달 수. 마지막 step_view 이후 submit 없으면 이탈로 간주 |
| 3 | `chat_step_choice` | 선택지 버튼 클릭 시 (목적/주제/성별/연령/직업/**직업선호**/통역/장소/시간/참여시기, + 환영 시작 버튼) | `step`, `value`: 선택한 값 | 어떤 선택이 많이 나오는지 |
