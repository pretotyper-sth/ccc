...
export default function SignupJpPage() {
  ...
  const validate = () => {
    let ok = true;
    if (!name.trim()) { setNameErr("お名前を入力してください。"); ok = false; } else setNameErr("");
    if (!email.trim()) { setEmailErr("メールアドレスを入力してください。"); ok = false; }
    else if (!isValidEmail(email)) { setEmailErr("正しいメール形式で入力してください。"); ok = false; }
    else setEmailErr("");
    if (!nation) { setNationErr("国籍を選択してください。"); ok = false; } else setNationErr("");
    return ok;
  };
