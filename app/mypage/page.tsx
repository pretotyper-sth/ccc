    ...
    return (
      <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: BG, display: "flex", flexDirection: "column" }}>
        <header style={{ padding: "14px 20px 12px", borderBottom: `1px solid ${BORDER}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Link href="/" style={{ fontSize: 12, color: TEXT_SEC }}>← 홈</Link>
        </header>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 24px", textAlign: "center" }}>
          <p style={{ margin: "0 0 6px", fontSize: 14, fontWeight: 700, color: "#111" }}>로그인이 필요합니다</p>
          <p style={{ margin: "0 0 28px", fontSize: 13, color: TEXT_TER, lineHeight: 1.7 }}>CCC 멤버만 이용할 수 있는 페이지예요.</p>
      ...
      <main style={{ flex: 1, padding: "24px 16px 120px" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 20, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.08)" }}>
            {[
              { label: "국적", value: user.nation },
              ...(user.ageGroup ? [{ label: "연령대", value: user.ageGroup }] : []),
              ...(user.gender ? [{ label: "성별", value: user.gender }] : []),
            ].map((r) => (
              <div key={r.label}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <p style={{ margin: 0, fontSize: 13, fontWeight: 800, color: "#111", letterSpacing: "-0.01em" }}>관심 세션</p>
            {wishlist.length > 0 && (
              <span style={{ fontSize: 12, color: TEXT_TER }}>{wishlist.length}개</span>
            )}
          </div>
          ...
          {wishlist.length === 0 ? (
            <div style={{
              ...
              <p style={{ margin: "0 0 4px", fontSize: 14, fontWeight: 800, color: "#111" }}>관심 세션을 찜해보세요</p>
              <p style={{ margin: "0 0 20px", fontSize: 13, color: TEXT_TER, lineHeight: 1.65 }}>
                마음에 드는 세션의 하트를 누르면<br />여기서 바로 확인하실 수 있어요
              </p>
              <Link href="/" style={{
